...
const compareArrays = (colorArray, textArray) => {
  return colorArray.some((item) => {
    return !!textArray.find((textItem) => textItem.x === item.x && textItem.y === item.y)
  })
}
...
const func = (arr) => {
  ...
  arr.forEach((item) => {
    if (Number(item) < 10) {
      temp = temp.length === 2 ? item + '' : temp + item;
  ...
  var loadPage = function (pageNum) {
    ...
    return doc.getPage(pageNum).then(function (page) {
      ...
      return page.getOperatorList().then(function (opList) {
        ...
        var save_x, save_y, save_width, save_height;
        var edges = [];
        ...
        return page.getTextContent().then(function (content) {
          ...
          sortedRects.forEach((item) => {
            const key = Math.round(item.offsetWidth);
            if (tempObj[key]) {
              tempObj[key] += 1;
            } else {
              tempObj[key] = 1;
          ...
          // ищем максимально повторяющиейся прямоугльник
          const objKeys = Object.keys(tempObj);
          const objValue = Object.values(tempObj);
          const maxCountVal = Math.max(...objValue);
          const maxCountValIndex = objValue.findIndex((item) => item === maxCountVal);
          const minWidthRect = Number(objKeys[maxCountValIndex]);

          const filteredRects = sortedRects.filter((item) => {
            const rounded = Math.round(item.offsetWidth);
            const plusOffset = rounded + 3;
            const minusOffset = rounded - 3;
            return minusOffset <= minWidthRect && plusOffset >= minWidthRect;
            // 26 <= 29 <= 32
          })
          // фильтранем высоты строк
          const rowsHeightsSet = new Set();
          const filteredByRowHeight = filteredRects.sort((a, b) => b.y - a.y);
          filteredByRowHeight.forEach((row) => {
            rowsHeightsSet.add(row.y);
          })

          const rowsHeights = [...rowsHeightsSet];

          // запишем в объекты прямоугольников высоты клеток
          const rectsWithRowHeights = filteredByRowHeight.map((row) => {
            const rowIndex = rowsHeights.findIndex((item) => row.y === item);
            const curLineY = rowsHeights[rowIndex];
            const nextLineY = rowsHeights[rowIndex + 1];
            // если это строка
            if (curLineY && nextLineY) {
              const deltaRowHeights = Math.abs(nextLineY - curLineY);
              return {
                ...row,
                deltaHeight: deltaRowHeights + row.height + 8
              }
            }
          }).filter((item) => item);
            ...
            rectsWithRowHeights.forEach((rectItem) => {
              if (x >= rectItem.xMin && x <= rectItem.xMax
                ...
                colorsArr.forEach((color) => {
                  const isInsideByX = x >= color.xMin && x <= color.xMax;
                  const isInsideByY = y >= color.yMin && y <= color.yMax;
                  if (isInsideByY && isInsideByX) {
                    isMarkedRect = true;
                  }
                })
              }
